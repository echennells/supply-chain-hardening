...
| Repo-local config in the other ecosystems (npm `./.npmrc`, uv `./uv.toml` / `pyproject [tool.uv]`, bun `bunfig.toml`, pnpm/yarn) | **In scope as a threat, not closable — the same shape as the cargo row above, generalized.** A project-directory config outranks the `~` and `/etc` files the role deploys, so a hostile repo (or an agent unpacking an archive) weakens hardening without touching what the role wrote. MEASURED (Ubuntu 26.04): a repo `./.npmrc` `ignore-scripts=false` re-runs a dependency postinstall in **non-PAM** contexts — under PAM the `NPM_CONFIG_IGNORE_SCRIPTS` env layer still wins (env > project); a repo `./uv.toml` `no-build=false` re-enables source builds in **every** context, because uv has no env layer the role can set without exporting `UV_CONFIG_FILE`, which *replaces* config discovery and breaks legitimate project settings. The mitigation is procedural: do not run package managers inside an untrusted repo, or do it in a container. See README "Limitations". |
