...
- **Composer script blocking goes through a PATH wrapper.** Composer has no host-wide config-file or env-var mechanism for disabling scripts — `COMPOSER_NO_SCRIPTS` and `"scripts-are-disabled": true` are not real composer concepts (composer ignores them). The role installs `/usr/local/bin/composer` as a wrapper that injects `--no-scripts --no-plugins` on every invocation; the real binary is preserved at `/usr/local/bin/composer-real`. Two known bypasses: (1) the wrapper is at a fixed path, so callers that invoke composer via `php /path/to/composer.phar` skip it (covered partially by `COMPOSER_SKIP_SCRIPTS` enumeration in `/etc/profile.d/` and `/etc/environment` for composer ≥ 2.9 and PAM-loaded shells); (2) `/usr/local/bin/composer-real` is the documented escape hatch for users who legitimately need scripts. Disable the wrapper entirely with `composer_path_wrapper=false` (restores the real binary at the wrap location on next apply).
