...
- **Matrix coverage is single-distro.** The cross-version test matrix at `tests/matrix/` verifies the role against 12 (PHP × composer) combinations, but only on Ubuntu 24.04. `meta/main.yml` claims support for Ubuntu 22.04 jammy and Debian 12 bookworm as well — those are unverified by the matrix. A passing matrix run is not evidence the role works on jammy or bookworm. See [tests/matrix/README.md](tests/matrix/README.md) → "Coverage gaps" for the full list of what the matrix doesn't verify.
