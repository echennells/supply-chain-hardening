...
- **The npm age gate requires npm ≥ 11.10.0.** `min-release-age` was added in npm 11.10.0 — below that npm does not know the key *at all* (it is absent from `npm config ls -l`), so neither `~/.npmrc`, `/etc/npmrc`, nor `NPM_CONFIG_MIN_RELEASE_AGE` enforce anything and the age gate is silently inactive. **Node 20 and Node 22 ship npm 10.x**, so this is the common case. The action now detects this and emits a `::warning::` instead of reporting success; `ignore-scripts`, `audit` and `save-exact` still apply. Note that `npm config get min-release-age` is *not* a valid way to check — npm echoes back unknown keys, reporting the value as healthy while enforcing nothing.
- **`pip` age-gating only covers the bare `pip` command.** The gate works by redirecting `/usr/local/bin/pip` through `uv` (pip itself has no age-gate setting). Anything that doesn't resolve that path bypasses it: `python -m pip`, a virtualenv's `pip`, and pyenv shims are all separate entry points. Verified 2026-08. This is structural — a PATH shim cannot intercept module or venv invocations. The `only-binary=:all:` sdist/`setup.py` block is *not* affected and still applies.
- **`pnpm_built_dependencies` does nothing on pnpm 11.** pnpm 11 rejects `onlyBuiltDependencies` in the global config file (`[WARN] … cannot be set in the global config file`) and falls back to blocking *all* dependency build scripts. It therefore fails safe — but the allowlist has no effect. To allow a specific package's build scripts on pnpm 11, put `onlyBuiltDependencies` in the project's `pnpm-workspace.yaml`. On pnpm 10 the allowlist works as documented.
